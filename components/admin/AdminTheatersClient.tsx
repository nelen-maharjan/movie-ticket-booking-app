...
import { ScreenForm, Theater, TheaterForm } from "@/lib/types/theaterScreen";

const SCREEN_TYPES = ["STANDARD", "IMAX", "4DX", "DOLBY"];

export function AdminTheatersClient({
  theaters,
}: {
  theaters: Theater[];
}) {
  ...
  const [screenForm, setScreenForm] = useState<ScreenForm>({
    name: "",
    totalRows: 8,
    totalCols: 12,
    screenType: "STANDARD",
  ...
  const handleTheater = (e: FormEvent) => {
    e.preventDefault();
    if (isPending) return;
      ...
      startTransition(async () => {
        try {
          await createScreen({
            ...
            totalCols: Number(screenForm.totalCols),
          });
        ...
        {theaters.map((theater) => (
          ...
          >
            <div
              ...
              onClick={() =>
                setExpanded(
                  expanded === theater.id ? null : theater.id
                )
              }
            >
            ...
            {expanded === theater.id && (
              <div className="border-t border-border p-5">
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {screen.screenType} ·{" "}
                          {screen.totalRows * screen.totalCols} seats
                        </p>
                      </div>
                    </div>
                  ))}
                ...
                {showScreenForm === theater.id && (
                  <form
                    onSubmit={handleScreen(theater.id)}
                    className="grid grid-cols-2 gap-4 p-4 bg-secondary/20 rounded-xl"
                  >
                        ...
                        {SCREEN_TYPES.map((t) => (
                          <button
                            ...
                            className={`px-2 py-1 rounded text-xs border transition-all ${
                              screenForm.screenType === t
                                ? "bg-cinema-gold/20 text-cinema-gold border-cinema-gold/40"
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        min="5"
                        max="30"
                        value={screenForm.totalCols}
                        onChange={(e) =>
                          setScreenForm((p) => ({
                            ...p,
                            totalCols: Number(e.target.value),
                          }))
                        }
                        required
                      />
                    ...
                    <p className="col-span-2 text-xs text-muted-foreground">
                      {screenForm.totalRows *
                        screenForm.totalCols}{" "}
                      total seats will be created
                    </p>
      ...
      {showTheaterForm && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md glass rounded-2xl p-6">
            ...
            <form
              onSubmit={handleTheater}
              className="space-y-4"
            >
              {[
                { id: "name", label: "Theater Name" },
                { id: "city", label: "City" },
                { id: "location", label: "Area/Locality" },
