...
import { calculateDynamicPrice, calculateLockExpiry, estimateDemandScore } from "@/lib/algorithms";
import { z } from "zod";

// Lock seats temporarily (10 min) to prevent concurrent booking conflicts
export async function lockSeats(showtimeId: string, seatIds: string[]) {
  ...
  if (takenSeats.length > 0) throw new Error("Some seats are already booked");
  ...
  if (activeLocks.length > 0) throw new Error("Some seats are temporarily held by another user");
  ...
  await Promise.all(
    seatIds.map(seatId =>
      db.seatLock.upsert({
        ...
        create: { seatId, showtimeId, userId: session.user!.id, expiresAt },
        update: { userId: session.user!.id, expiresAt },
      })
    )
  );
...
// Get dynamic prices for seats
export async function getSeatPrices(showtimeId: string, seatIds: string[]) {
  ...
  const prices = seats.map(seat => {
    ...
    return { seatId: seat.id, price, type: seat.type };
  });
...
// Create booking
export async function createBooking(showtimeId: string, seatIds: string[]) {
  const session = await getServerSession(authOptions);
  if (!session?.user) throw new Error("Not authenticated");

  // Verify locks belong to user
  const now = new Date();
  ...
  if (locks.length !== seatIds.length) throw new Error("Seat locks expired. Please try again.");
  ...
  const booking = await db.$transaction(async (tx) => {
    // Double-check no one else booked
    ...
    if (conflicting) throw new Error("Seat conflict detected. Please choose different seats.");
    ...
    const newBooking = await tx.booking.create({
      data: {
        ...
        status: "CONFIRMED",
        bookingSeats: {
          create: seatPrices.map(({ seat, price }) => ({ seatId: seat.id, price })),
        },
      },
      include: { bookingSeats: true, showtime: { include: { movie: true, screen: { include: { theater: true } } } } },
    });
    ...
    // Delete locks
    await tx.seatLock.deleteMany({ where: { seatId: { in: seatIds }, showtimeId } });
...
export async function cancelBooking(bookingId: string) {
  ...
  if (booking.showtime.startTime <= new Date()) throw new Error("Cannot cancel past shows");

  await db.$transaction([
    db.booking.update({ where: { id: bookingId }, data: { status: "CANCELLED" } }),
    db.showtime.update({
      where: { id: booking.showtimeId },
      data: { availableSeats: { increment: booking.bookingSeats.length } },
    }),
  ]);
